...
// ---------- disk capacity ----------
// fs.statfsSync gives the same numbers `df -k /` reports, without spawning a
// subprocess on every sample. The percentage deliberately matches df's "Use%"
// column (used / (used + available)) rather than a whole-device ratio, so the
// dashboard shows the figure operators already recognise.
function diskUsage(target) {
  try {
    const s = fs.statfsSync(target || '/');
    const bsize = Number(s.bsize) || 0;
    if (!bsize) return null;
    const total = bsize * s.blocks;
    const free = bsize * s.bfree;
    const avail = bsize * s.bavail;
    ...
    return {
      ...
      usedB: used,
      availB: avail,
      usePct: denom > 0 ? Math.round((used / denom) * 1000) / 10 : null
    };
  } catch (_) {
    return null;
  }
...
function sample(nowMs) {
  ...
  const out = {
    ...
    load1: load ? load.load1 : null,
    load5: load ? load.load5 : null,
    load15: load ? load.load15 : null,
    conns: {
...
module.exports = {
  ...
  parseTcpStates, parseTcpStates6, parseVmstat, diskUsage, sample, latest, _reset
};
