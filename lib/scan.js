...
async function runScan() {
  ...
  for (const a of apps) {
    ...
    items.push(policy.applyOverrides({
      ...
      // When the master switch is off, advertise no actions at all, exactly as
      // if the app had no allow entry, so the UI greys the buttons out instead
      // of rendering live-looking controls that only 403 when clicked.
      // The server-side gate (server.js actionsEnabled) stays authoritative.
      actions: (isPanel || pol.actions_enabled !== true) ? [] : policy.actionsFor(pol, a.name),
