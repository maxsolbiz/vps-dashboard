...
const { ensureDir, writePrivate } = require('./fsutil');
...
// Flips ONLY policy.actions_enabled. Every other key is preserved exactly, and
// the write is verified by re-reading the file before reporting success. This
// deliberately cannot touch allow/deny/logs_disabled: the toggle arms the
// master switch, it never widens who is allowed to act.
function setActionsEnabled(enabled) {
  const want = enabled === true;
  let raw;
  try {
    raw = JSON.parse(fs.readFileSync(config.policyPath, 'utf8'));
  } catch (e) {
    return { ok: false, error: 'policy file is unreadable or malformed' };
  }
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
    return { ok: false, error: 'policy file is not a JSON object' };
  }
  const before = JSON.stringify(raw);
  const next = { ...raw, actions_enabled: want };
  try {
    writePrivate(config.policyPath, `${JSON.stringify(next, null, 2)}\n`);
  } catch (_) {
    return { ok: false, error: 'could not write policy file' };
  }
  // Verify the round-trip and that nothing else moved.
  let after;
  try {
    after = JSON.parse(fs.readFileSync(config.policyPath, 'utf8'));
  } catch (_) {
    return { ok: false, error: 'policy file did not round-trip as valid JSON' };
  }
  if (after.actions_enabled !== want) {
    return { ok: false, error: 'actions_enabled did not persist' };
  }
  const stripped = { ...after };
  delete stripped.actions_enabled;
  const strippedBefore = { ...JSON.parse(before) };
  delete strippedBefore.actions_enabled;
  if (JSON.stringify(stripped) !== JSON.stringify(strippedBefore)) {
    return { ok: false, error: 'refusing: another policy key changed' };
  }
  return { ok: true, actions_enabled: want };
}

module.exports = { loadPolicy, getPolicyError, getLastGood, loadOverrides, saveOverrides, logsEnabled, applyOverrides, actionsFor, normalizeAllow, setActionsEnabled, CATEGORIES, META_FIELDS, VALID_ACTIONS };
