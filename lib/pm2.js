...
// Poll until the process is online (or the deadline passes). Read-only.
async function waitOnline(name, timeoutMs) {
  const started = Date.now();
  for (;;) {
    let live = null;
    try {
      live = (await jlist()).find((p) => p.name === name) || null;
    } catch (_) { live = null; }
    if (live && live.status === 'online') {
      return { verified: true, waited_ms: Date.now() - started, pid: live.pid };
    }
    if (Date.now() - started >= timeoutMs) {
      return { verified: false, waited_ms: Date.now() - started, pid: live ? live.pid : 0 };
    }
    await new Promise((r) => setTimeout(r, 500));
  }
}

module.exports = { jlist, action, parseJlist, clearCache, waitOnline, NAME_RE, ACTION_VERBS };
